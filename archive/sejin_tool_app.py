...
def get_sequence_files(uploaded_files):
    """
    업로드된 JSON 파일들의 이름을 정렬하여 반환합니다.
    """
    # 파일 이름과 경로를 저장할 리스트
    sequence_files = []
    ...
    for i, uploaded_file in enumerate(uploaded_files):
        if i % 3 == 0:
            ...
            # 리스트에 파일 이름과 파일 객체 추가
            sequence_files.append((file_name, uploaded_file))

    # 파일 이름을 번호 순서대로 정렬 (예시: "000000000120"에 따라 정렬)
    sequence_files.sort(key=lambda x: int(re.search(r'\d{12}', x[0]).group()))
